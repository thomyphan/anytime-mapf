...
InitLNS::InitLNS(const Instance& instance, vector<Agent>& agents, double time_limit,
         const string & replan_algo_name, const string & init_destroy_name, int neighbor_size, int screen, const string & bandit_algorithm_name, int neighborhoodSizes) :
         BasicLNS(instance, time_limit, neighbor_size, screen, bandit_algorithm_name, neighborhoodSizes, INIT_COUNT), agents(agents), replan_algo_name(replan_algo_name),
         path_table(instance.map_size, agents.size()), collision_graph(agents.size()), goal_table(instance.map_size, -1)
 {
     ...
     ucb1Constant = 1000;
     if (init_destroy_name == "Adaptive")
     {
        ALNS = true;
        heuristicBanditStats.destroy_weights.assign(INIT_COUNT, 1);
        heuristicBanditStats.destroy_weights_squared.assign(INIT_COUNT, 1);
        heuristicBanditStats.destroy_counts.assign(INIT_COUNT, 0);
        decay_factor = 0;
        reaction_factor = 0;
        if(numberOfNeighborhoodSizeCandidates > 0)
        {
            for(int index = 0; index < INIT_COUNT; index++)
            {
     ...
     else if (init_destroy_name == "Target")
         init_destroy_strategy = TARGET_BASED;
     else if (init_destroy_name == "Collision")
         init_destroy_strategy = COLLISION_BASED;
     else if (init_destroy_name == "Random")
         init_destroy_strategy = RANDOM_BASED;
     else
     {
         cerr << "Init Destroy heuristic " << init_destroy_name << " does not exists. " << endl;
         exit(-1);
     }
...
bool InitLNS::run()
{
    ...
    while (runtime < time_limit and num_of_colliding_pairs > 0)
    {
        ...
        if (neighbor.old_colliding_pairs.empty()) // no need to replan
        {
            ...
            {
                updateDestroyAndNeighborhoodWeights(0, false);
            }
            continue;
        }
        ...
        if (ALNS) // update destroy heuristics
        {
            const bool condition = neighbor.colliding_pairs.size() < neighbor.old_colliding_pairs.size();
            const double value = (double)(neighbor.old_colliding_pairs.size() - neighbor.colliding_pairs.size());
